...
echo "Copiando todos los archivos a la máquina local..."
rsync -avz --exclude={"/dev/*","/proc/*","/sys/*","/tmp/*","/run/*","/mnt/*","/media/*","/lost+found"} -e "ssh" "$USER@$IP_CENTOS:$RUTA_COPIA" "$RUTA_LOCAL"

# 5. Crear directorio de destino en Ubuntu
echo "Creando directorio de destino en Ubuntu..."
ssh "$USER@$IP_UBUNTU" "mkdir -p $RUTA_DESTINO"

# 6. Copiar desde la máquina local al servidor Ubuntu
echo "Copiando todos los archivos al servidor Ubuntu..."
rsync -avz "$RUTA_LOCAL/" "$USER@$IP_UBUNTU:$RUTA_DESTINO"
